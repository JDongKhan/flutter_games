...
class WeldJointWorld extends Forge2DWorld with TapCallbacks, HasGameReference<Forge2DGame> {
  ...
  Future<void> onLoad() async {
    ...
    final leftPillar = Box(
      startPosition: game.screenToWorld(Vector2(50, game.size.y))..y -= pillarHeight / 2,
      width: pillarWidth,
      height: pillarHeight,
    ...
    final rightPillar = Box(
      startPosition: game.screenToWorld(Vector2(game.size.x - 50, game.size.y))..y -= pillarHeight / 2,
      width: pillarWidth,
      height: pillarHeight,
      bodyType: BodyType.static,
      color: Colors.white,
  ...
  Future<void> createBridge(
    ...
    final sectionWidth = ((leftPillar.center.x.abs() + rightPillar.center.x.abs() + pillarWidth) / sectionsCount).ceilToDouble();
    Body? prevSection;
